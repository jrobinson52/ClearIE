    ...
    switch ($userMenuChoice) {
      ...
      3{#-option 2--------------------------------------------------------------------
       ...
       clear
        ...
        Write-Host " "
        Write-Host " "
        Write-Host -ForegroundColor Green "Calculating space to be recovered"
        ...
        #determine if answer should be in MB or GB

        if (("{0:N2}" -f ($colItems.sum / 1GB)) -le 0.90)
          {
           "{0:N2}" -f ($colItems.sum / 1MB) + " MB of space will be recovered"              
          }
        else
          {
            "{0:N2}" -f ($colItems.sum / 1GB) + " GB of space will be recovered"
          }
      ...
      4{#-option 3-----------------"C:\Windows\Temp\*" delete-----"C:\Users\421067\AppData\Local\Temp\*" Delete--------
       ...
       clear
        ...
        #calculate size of files to be deleted               
        $colItems = (Get-ChildItem C:\Windows\temp -recurse | Measure-Object -property length -sum)
        $colItems1 = (Get-ChildItem C:\Users\*\AppData\Local\Temp -recurse | Measure-Object -property length -sum)

        #determine if answer should be in MB or GB

         if (("{0:N2}" -f (($colItems.sum + $colItems1.sum) / 1GB)) -le 0.90)
           {
            "{0:N2}" -f (($colItems.sum + $colItems1.sum) / 1MB) + " MB of space will be recovered"              
           }
         else
           {
             "{0:N2}" -f (($colItems.sum + $colItems1.sum) / 1GB) + " GB of space will be recovered"
           }
